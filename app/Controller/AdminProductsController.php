...
class AdminProductsController extends AdminContentController {
    ...
    public function index($parent_id = '') {
        ...
        if ($limit = Hash::get($this->request->params, 'named.limit')) {
            ...
            $this->Settings->save(array('id' => 1, 'limit' => intval($limit)));
        } elseif ($settings['Settings']['limit']) {
            $this->paginate['limit'] = $settings['Settings']['limit'];
        }
        $filter['limit'] = $this->paginate['limit'];

        if ($page = Hash::get($this->request->params, 'named.page')) {
            $this->paginate['page'] = $page;
            $this->Settings->save(array('id' => 1, 'page' => intval($page)));
        } elseif ($settings['Settings']['page']) {
            $this->paginate['page'] = $settings['Settings']['page'];
        }
